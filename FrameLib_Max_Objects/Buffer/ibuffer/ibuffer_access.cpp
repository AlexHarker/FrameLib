...
void ibuffer_read_format(const ibuffer_data& buffer, T *out, U *positions, intptr_t n_samps, long chan, T mul, htl::interp_type interp)
{
    ...
    {
        case PCM_FLOAT:     htl::table_read(fetch_float(buffer, chan), out, positions, n_samps, mul, interp);   break;
        case PCM_INT_16:    htl::table_read(fetch_16bit(buffer, chan), out, positions, n_samps, mul, interp);   break;
        case PCM_INT_24:    htl::table_read(fetch_24bit(buffer, chan), out, positions, n_samps, mul, interp);   break;
        case PCM_INT_32:    htl::table_read(fetch_32bit(buffer, chan), out, positions, n_samps, mul, interp);   break;
    }
}
