...
mob::mob(FrameLib_Proxy *proxy) : mNumAudioIns(0), mNumAudioOuts(0), mProxy(proxy) // this is an initialisation list
{
FrameLib_Global::get(&mGlobal);
FrameLib_Context context(mGlobal, nullptr);
FrameLib_Parameters::AutoSerial parameters;
mObject = new FrameLib_Expand<FrameLib_Interval>(context, nullptr, mProxy, 1);
}
...
//        std::string object = object_classname(mUserObject)->s_name;
        ...
        // This are extra args for the info message. You can probably remove this at some point
//        while (ac--)
//        {
