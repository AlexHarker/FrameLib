...
template <class V> void unzipComplex(float *input, FFT_SPLIT_COMPLEX_F *output, uintptr_t half_length)
{
    vDSP_ctoz((COMPLEX *) input, (vDSP_Stride) 2, output, (vDSP_Stride) 1, half_length);
}

template <class V> void unzipComplex(double *input, FFT_SPLIT_COMPLEX_D *output, uintptr_t half_length)
{
